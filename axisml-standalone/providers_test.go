...
func TestConfigResourceCatalog_MultiplePools(t *testing.T) {
	...
	gpu := validPool()
	gpu.Name = "gpu"
	cat := standalone.NewConfigResourceCatalog(validPool(), gpu)
...
func TestStaticTenantStore_MultipleTenants(t *testing.T) {
	...
	store := standalone.NewStaticTenantStore(validTenant(), teamA)
...
func TestErrCapabilityUnavailableIsSentinel(t *testing.T) {
	err := standalone.NewConfigResourceCatalog(validPool()).Create(context.Background(), &cmv1alpha1.ResourcePool{})
