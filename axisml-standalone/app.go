...
// App is an assembled axisml-standalone: the shared database, the in-process Standalone
...
// never both.
var ErrLoopsAlreadyStarted = errors.New("axisml-standalone: background loops already started")

// New is the axisml-standalone composition root. It resolves the inputs (Settings,
...
func New(ctx context.Context, cfg Config, opts ...Option) (app *App, err error) {
	...
	gpuDevices, err := dockerruntime.ResolveGPUDevices(cfg.GPU.Devices)
	if err != nil {
...
// alongside these.
//
// axisml-standalone's middleware chain (request id, access log, recovery, both
// services' identity stamping, shared RFC 7807 error rendering) is applied to a
// child group, so it wraps ONLY axisml-standalone's routes — never the host's own
...
func (a *App) Handler() http.Handler {
	a.engineOnce.Do(func() {
		// Force ReleaseMode only for axisml-standalone's own engine; a native-gin host
...
func (a *App) Serve(ctx context.Context) error {
	...
	go func() {
		a.log.Info("axisml-standalone listening", "addr", a.settings.APIBindAddress)
...
// Run is the binary's convenience entry point: assemble with the fixed standalone
...
// backs the `axisml-standalone migrate` subcommand for an explicit pre-flight step.
