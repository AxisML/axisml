...
func TestOpenAPISpecFull(t *testing.T) {
	...
	assert.Equal(t, "AxisML Standalone API", info["title"])
...
func TestOpenAPISpecScopedPrunesPathsAndSchemas(t *testing.T) {
	...
	// Dropped: everything outside the allowlist, including the standalone probes and
...
func TestOpenAPISpecWithVersion(t *testing.T) {
	...
	assert.Equal(t, "AxisML Standalone API", info["title"], "WithVersion leaves title at the default")
}
