...
type Runtime struct {
	...
	log    logr.Logger
	events *eventRing
	// registryAuth resolves the Docker Engine RegistryAuth request field from
	// cfg.DockerConfigFile. Nil preserves anonymous pulls.
...
func New(cli *client.Client, cfg Config, log logr.Logger) *Runtime {
	return &Runtime{
		...
		registryAuth: newRegistryAuthResolver(cfg.DockerConfigFile),
		cancelled:    map[string]bool{},
...
// tenant label mirrors the workload namespace: in standalone the tenant scope is the
