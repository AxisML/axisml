...
- Design docs: each layer owns its per-component docs under `<layer>/docs/`. Cross-cutting design docs stay in `docs/system_design/` — `high_level_design.md` (system-level overview) and `deployment.md`; DB schema is split per-layer under each `<layer>/docs/system_design/database.md`. Generated API specs live in each layer's `docs/apis/`. Other doc trees: `docs/development_workflow.md` (the single dev guide — setup, build/test, testing layers); product/UX docs live in `axisml-platform/docs/product_design/` (incl. an interactive `prototype/`). `DESIGN.md` at the root is the **frontend** visual design system (Vercel Geist style), not the system design.
...
- `make doc-gen` (root, all specs) or per-component, e.g. `make -C axisml-system compute-service-doc-gen`, regenerates the spec(s). Platform's is `make -C axisml-platform doc-gen`.
