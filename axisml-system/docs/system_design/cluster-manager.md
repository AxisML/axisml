...
集群级 admin REST 抽象层：把 admin 视角的 K8s 写 / 读收敛成简单 REST，让上游全程不直接调 Kubernetes API。承载 `ResourcePool` CRD 的 CRUD（含内嵌 `units[]`）、集群级 `Tenant` CRD 的 CRUD（含配额「资源单元 × 数量」折算为 ElasticQuota `min`/`max` 写入 CR），以及通用 `Volume` 的生命周期（持久卷 PVC 的 create / delete）。仅接受 Platform 内部调用并信任 `X-Axisml-User` 透传；Volume 由 Platform 提前创建，compute 仅在工作负载里引用挂载，不调本服务。
...
| Volume REST（持久卷 PVC 的 create / delete，幂等；卷名 / size / storageClass 由 Platform 携带） | 决定卷挂到哪个工作负载 / 卷内容（Platform 提前建卷，compute 在 Pod 模板里引用挂载） |
...
| Volume | 通用持久卷，直接物化为 namespace-scoped PVC | `(namespace, name)` | 无 CR；卷名 / size / storageClass 由调用方携带；create / delete 幂等；§3.4 |

> 无独立 ResourceUnit CR——`units` 是 `ResourcePool.spec.units[]` 数组项，与 pool 同生灭、原子编辑。无独立配额 CRD——内联 `Tenant.spec.quotas[]`，折算后写入（§3.3）。Volume 亦无 CR——直接是一个受管 PVC（Lite 形态下是受管 Docker 卷）；卷的语义命名（如工作区 `axisml-ws-<svc>-data`）由调用方 Platform 决定，挂载由 compute 在 Pod 模板里以 PVC 引用完成，本服务只按 `(namespace, name)` 物化 / 回收（§3.4）。
...
{ "namespace": "axisml-team-alpha",      // 物理 K8s Namespace（= 租户的 spec.namespace.name）
  "name": "axisml-ws-notebook-1-data",   // 卷名，调用方负责命名（PVC 名 / Docker 卷名）
  "size": "50Gi",                        // Kubernetes Quantity；K8s 形态必填
  "storageClass": "" }                   // 空 = 集群默认 StorageClass
```

K8s 形态：`Volume` 直接物化为该 namespace 下的 `PersistentVolumeClaim`（`ReadWriteOnce`，`spec.resources.requests.storage = size`，可选 `storageClassName`），打 `axisml.io/*` label 供 selector 区分。Lite 形态：物化为受管 Docker named volume（size / storageClass 被接受但忽略——单机卷按需增长、无 class）。

**契约**：`create` 幂等（卷已存在 = 成功）；`delete` 幂等（卷不存在 = 成功）；`namespace` / `name` 必填；K8s 形态 `size` 必填且须为合法 Quantity。本服务不感知卷的用途——「这是工作区的卷」「应挂到哪个 Pod」全在调用方 Platform；本服务只按 `(namespace, name)` 物化 / 回收。
...
| 操作 | 内部行为 |
| --- | --- |
| 创建 POST `/api/v1/volumes` | 校验 `namespace` / `name` 非空、`size` 合法 Quantity；K8s create PVC，`AlreadyExists` 视为成功（幂等）；返回 `Volume` |
| DELETE `/api/v1/volumes/{namespace}/{name}` | delete PVC，`NotFound` 视为成功（幂等 204） |

唯一调用方是 Platform（提前创建、回收工作区等所需的持久卷时）。本服务跨 namespace 写 PVC，故 RBAC 含 `persistentvolumeclaims` `create/get/list/delete`（§8）。无展开、无折算、无 CR——单次 K8s API 调用即返回。
...
| 物化 K8s 资源 | `PersistentVolumeClaim`（核心 v1，namespace-scoped）——Volume REST 的落地形态；幂等 create / delete | — |
...
| Platform | 唯一外部调用方；admin 域 UI 入口（资源池 + 租户），并提前创建 / 回收持久卷 |
...
| RBAC scope | ClusterRole：`resourcepools` / `tenants.axisml.io`（`get/list/watch/create/update/patch/delete`）、跨 ns `persistentvolumeclaims`（`get/list/create/delete`，Volume 物化）、`events` `create/patch` |
