...
// MLService onto PG via the shared writeback helpers — the same mapping the
// Kubernetes informer uses.
...
func (p *StatusPoller) runOnce(ctx context.Context) {
	...
	if err != nil {
		p.log.Error(err, "find observable services")
		return
	}
	for i := range rows {
		row := &rows[i]
		key := types.NamespacedName{Namespace: row.Namespace, Name: row.Name}
		observed, err := p.runtime.ObserveMLService(ctx, key)
		if apierrors.IsNotFound(err) {
			reflectGone(ctx, p.repo, row)
			continue
		}
		if err != nil {
			p.log.Error(err, "observe MLService", "name", row.Name)
			continue
		}
		reflectObserved(ctx, p.repo, row, desiredReplicas(row), observed)
	}
}
