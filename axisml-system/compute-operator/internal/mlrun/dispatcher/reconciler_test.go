...
type recordingHandler struct {
	...
	mapStatusPhase  axisv1alpha1.MLRunPhase
}
...
func (h *recordingHandler) Validate(*axisv1alpha1.MLRun) field.ErrorList { return h.validateErrs }

func (h *recordingHandler) Reconcile(ctx context.Context, c client.Client, mlJob *axisv1alpha1.MLRun) (any, axishandler.ReconcileResult, error) {
...
func (h *recordingHandler) Sweep(ctx context.Context, c client.Client, mlJob *axisv1alpha1.MLRun) (int32, error) {
	h.sweepCalled = true
...
func newTerminalMLRun() *axisv1alpha1.MLRun {
	mlj := &axisv1alpha1.MLRun{
		...
		Status: axisv1alpha1.MLRunStatus{
...
func TestReconcile_TerminalShortCircuits(t *testing.T) {
	...
	r := &MLRunReconciler{Client: c, Registry: reg}
	...
	if h.reconcileCalled {
		t.Fatalf("handler.Reconcile must NOT be called when MLRun is already terminal")
	}
	if !h.sweepCalled {
...
func newPendingMLRun() *axisv1alpha1.MLRun {
	return &axisv1alpha1.MLRun{
		ObjectMeta: metav1.ObjectMeta{
			Name:       "fresh",
			Namespace:  "tnt",
			Generation: 1,
...
func TestReconcile_UnknownBackendFails(t *testing.T) {
	...
	r := &MLRunReconciler{Client: c, Registry: NewRegistry()}
	if _, err := r.Reconcile(context.Background(), ctrl.Request{
		NamespacedName: client.ObjectKey{Namespace: mlj.Namespace, Name: mlj.Name},
	}); err != nil {
		t.Fatalf("Reconcile: %v", err)
...
func TestSpecFingerprint(t *testing.T) {
	...
	mutations := []struct {
		name   string
		mutate func(*axisv1alpha1.MLRun)
	}{
		{"backend.name", func(m *axisv1alpha1.MLRun) { m.Spec.Backend.Name = "other" }},
		{"backend.engine", func(m *axisv1alpha1.MLRun) { m.Spec.Backend.Engine = "podgroup" }},
		{"role replicas", func(m *axisv1alpha1.MLRun) { m.Spec.Roles[0].Replicas = 8 }},
		{"role name", func(m *axisv1alpha1.MLRun) { m.Spec.Roles[0].Name = "ps" }},
		{"template image", func(m *axisv1alpha1.MLRun) { m.Spec.Roles[0].Template.Image = "img:v2" }},
		{"scheduling quota", func(m *axisv1alpha1.MLRun) { m.Spec.Scheduling.Quota = "other-quota" }},
		{"runPolicy.activeDeadline", func(m *axisv1alpha1.MLRun) {
			d := int64(60)
...
// reconcileImmutabilityRejection seeds an MLRun whose applied-spec
// annotation reflects `prior` and whose live spec is `current`, then
// runs Reconcile. Returns the post-reconcile MLRun.
func reconcileImmutabilityRejection(t *testing.T, prior, current *axisv1alpha1.MLRun) axisv1alpha1.MLRun {
	t.Helper()
	s := newReconcilerScheme(t)
...
func TestReconcile_RejectsRoleTopologyMutation(t *testing.T) {
	...
	prior := newPendingMLRun() // replicas=2
	current := newPendingMLRun()
