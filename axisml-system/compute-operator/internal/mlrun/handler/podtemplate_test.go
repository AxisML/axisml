...
func validMLRun() *axisv1alpha1.MLRun {
...
func TestEnsureRequiredCRLabels_MissingKey(t *testing.T) {
	mlj := &axisv1alpha1.MLRun{}
	mlj.Labels = map[string]string{axislabels.RunIDLabel: "job-1"} // missing quota
	errs := EnsureRequiredCRLabels(mlj)
	if len(errs) != 1 {
...
func TestEnsureRequiredCRLabels_AllPresent(t *testing.T) {
	if errs := EnsureRequiredCRLabels(validMLRun()); len(errs) != 0 {
...
func TestInjectAxisMLLabels_RejectsMissingJobID(t *testing.T) {
	mlj := validMLRun()
	delete(mlj.Labels, axislabels.RunIDLabel)
...
func TestOwnerRef_PointsToMLRun(t *testing.T) {
	mlj := validMLRun()
	ref := OwnerRef(mlj)
	if ref.Kind != "MLRun" {
	...
	if ref.Name != mlj.Name || ref.UID != mlj.UID {
		t.Errorf("ref does not match MLRun: %+v", ref)
	}
