...
from clients.computeservice.api.ml_runs import create_ml_run, delete_ml_run, get_ml_run_metrics
...
def test_mlrun_metrics(harness, cfg, tenant):
    ...
    try:
        m = get_ml_run_metrics.sync_detailed(ns, name, client=harness.compute_service, metric="cpu_util", range_="15m")
        ...
        bad = get_ml_run_metrics.sync_detailed(ns, name, client=harness.compute_service, metric="request_rate", range_="15m")
...
def test_mlservice_metrics(harness, cfg, tenant):
    ...
    assert r.status_code in (200, 201), r.content
    try:
        m = get_ml_service_metrics.sync_detailed(ns, name, client=harness.compute_service, metric="mem_util", range_="15m")
        assert m.status_code == 200, m.content
