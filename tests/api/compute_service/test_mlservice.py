...
def test_mlservice_list_patch_logs(harness, cfg, tenant):
    ...
    try:
        ...
        assert any(it.name == name for it in lst.parsed.items), "created service absent from list"

        # Patch display fields, then read them back.
        p = patch_ml_service.sync_detailed(
            ns, name, client=harness.compute_service, body=MLServicePatchRequest(display_name="Renamed Svc")
        )
        ...
        logs = get_ml_service_pod_logs.sync_detailed(ns, name, pods.parsed.items[0].name, client=harness.compute_service)
