...
from lib.harness import elastic_quota_name
from lib.polling import eventually


def admin_client(base_url: str, token: str) -> AuthenticatedClient:
    return AuthenticatedClient(base_url=base_url, token=token, raise_on_unexpected_status=False)


def create_tenant_with_quota(client: AuthenticatedClient, identifier: str, admin_username: str, cfg: config.Config) -> None:
    ct = create_tenant.sync_detailed(
        client=client,
        body=TenantCreateRequest(identifier=identifier, kubernetes_namespace=identifier, display_name=identifier, initial_admin=admin_username),
    )
    if ct.status_code not in (200, 201):
        raise AssertionError(f"seed tenant {identifier}: {ct.status_code}: {ct.content!r}")
    ...
    def put_quota():
        q = create_tenant_quota.sync_detailed(
            identifier, client=client, body=QuotaCreateRequest(pool=cfg.default_pool, units=[QuotaUnit(unit_name=cfg.default_unit, quantity=1)])
        )
        assert q.status_code in (200, 201), f"seed quota for {identifier}: {q.status_code}: {q.content!r}"
...
def create_simple_job(client: AuthenticatedClient, tenant: str, name: str, cfg: config.Config) -> None:
    spec = JobSpec(
        backend=Backend(name=BackendName.NATIVE, engine="job"),
        pool_name=cfg.default_pool,
        unit_name=cfg.default_unit,
        quota=elastic_quota_name(tenant, cfg.default_pool),
        roles=[MLRunRole(name="worker", replicas=1, template=RoleTemplate(image=cfg.mlrun_image, command=["sh", "-c", "echo hi"]))],
    )
    r = create_job.sync_detailed(client=client, x_axisml_tenant=tenant, body=JobCreateRequest(name=name, display_name=name, spec=spec))
    if r.status_code not in (200, 201):
        raise AssertionError(f"seed job {name}: {r.status_code}: {r.content!r}")


def trigger_job_run(base_url: str, token: str, tenant: str, job: str, cfg: config.Config) -> None:
    ...
    r = httpx.post(
        ...
        json={"poolName": cfg.default_pool, "unitName": cfg.default_unit, "quota": elastic_quota_name(tenant, cfg.default_pool)},
        timeout=15.0,
    )
