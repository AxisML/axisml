...
```
Page Head:  // training / workspaces   工作区。        [+ 新建工作区]
...
```
Page Head:  // serving / services   在线服务。        [+ 新建服务]
...
├───────────────┼────────┼──────────┼─────────────────────────┼──────────────────────┼────────────────────┤
│ rt-chat       │ 灰度    │ ◐ 灰度中 │ chat-v1 ▰▰▰▰▰▰▰▰▱ 90    │ /services/team-a/cha…│调流量 提升 回滚 详情│
│               │        │          │ chat-v2 ▰▱▱▱▱▱▱▱▱ 10    │                      │                    │
│ rt-embed      │ 加权    │ ● 生效中 │ embed-a 50 ▏ embed-b 50 │ /services/team-a/emb…│调流量 详情 删除     │
│ rt-rerank     │ 灰度    │ ○ 未就绪 │ rerank-v2 0 ▏稳定缺失    │ —                    │详情 删除           │
└───────────────┴────────┴──────────┴─────────────────────────┴──────────────────────┴────────────────────┘
...
**操作**:调整流量(改权重 / 灰度百分比)· 提升 / 回滚(仅灰度模式)· 详情 · 删除。
...
- **调整流量** — 弹窗:加权模式编辑各后端权重(实时 `Σ=100`);灰度模式拖灰度百分比 slider(0–100)。
...
与 §8.7 一致;新建 / 调整流量 / 提升 / 回滚 / 删除限 `owner` 或 `tenant-admin`,普通成员只读。
