...
that L1 didn't. If full-stack coverage is needed in the future, add a
self-hosted-runner job or a nightly cron rather than re-introducing it
on every PR.
...
- "Does this REST endpoint return the right status code / body shape?" → L1 integration (drive the gin engine in-process via `httptest`).
...
- **Build tag**: `//go:build integration` for L1. Default `go test ./...`
...
- **HTTP API tests**: drive the gin engine in-process via
  `engine.ServeHTTP(rr, req)` — see `components/compute/test/integration/httptest_helpers_test.go`
...
Unit and L1 integration both produce coverage profiles.
