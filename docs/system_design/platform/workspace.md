...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `id` | uuid PK | URL 路径中的 `{id}` 即此列；同时 deterministic 派生 PVC 名 `axisml-ws-<id8>-data`（取前 8 字符） |
| `tenant_id` | uuid REFERENCES `tenants(id)` | RBAC 裁剪 + `compute_namespace` 解析的入口键；多租户共享同一 K8s namespace 时也能按租户分组 |
| `service_id` | uuid uniq (partial, `WHERE deleted_at IS NULL`) | Compute `services.id`（[compute.md §7.2](../core/compute.md#72-数据模型) 的 PG 主键），也是 Compute 提供的稳定下游 join key（[§3.4](#34-对-core-层的硬依赖必须同-pr-推进)） |
| `created_at` / `updated_at` / `deleted_at` | timestamptz | GORM 自动维护；`deleted_at IS NOT NULL` 表示已删除 |
...
- **不存 `pvc_name` / `pvc_size`**：`pvc_name = "axisml-ws-" + id 前 8 字符 + "-data"` 由 id 派生；`pvc_size` 通过实时 `kubectl get pvc` 读出（详情页才取）。
- 索引：unique partial `(service_id) WHERE deleted_at IS NULL`、`(tenant_id, deleted_at)`。

### 3.2 显式不存的字段（实时穿透或派生）

| 字段 | 来源 |
| --- | --- |
| `mlservice_name` / `compute_namespace` | `compute.GetServiceByID(service_id)` 返回的 `name` / `namespace`（[§3.4](#34-对-core-层的硬依赖必须同-pr-推进) 新增端点） |
| `display_name` / `description` / `owner_user` / `pool_id` / `resource_unit_id` / `replicas` / `ready_replicas` / `endpoint` / `status` / `message` | 同一次 `compute.GetServiceByID` 响应字段 |
| `image` / `imageRef` | `services.spec.roles[0].template.image`（Compute 已在 [compute.md §7.2](../core/compute.md#72-数据模型) `spec` jsonb 快照中持有） |
...
| `last_started_at` / `last_stopped_at` | 阶段一直接用 `services.updated_at`；闲时回收功能上线时若需更精细，再单独建表 |
...
3. `compute.ListServicesByIDs(service_ids)` 一次批量拿全部 service 视图（[§3.4](#34-对-core-层的硬依赖必须同-pr-推进) 新增端点）；
4. 内存中按 `service_id` join，对普通用户再按 `services.owner_user == current_user.username` 二次过滤；
5. 返回带派生 `status` / `desired_state` / `access_url` 的 Workspace DTO 列表。

这是 service_id 寻址相对 `(namespace, name)` 寻址的天然优势——Platform 不必先把 `tenant_id → compute_namespace` 解析出来再 list / filter。
...
#### B. Compute service 提供 id-based 寻址（Platform→Compute join key）
...
- 列表渲染：按 [§3.3](#33-列表查询路径) 走「先 PG 取 service_id 集合 → Compute 批量 GET → 内存 join」的 N+1 优化路径。
- 列表可见性：
  - `system-admin`：全集群 `workspaces`（不限 tenant_id）；
  - `tenant-admin@self`：`workspaces.tenant_id IN (RBAC 可见租户)`；
  - `user`：上一条基础上再按 `services.owner_user == current_user.username` 二次过滤。
...
| 租户 | Platform RBAC + `workspaces.tenant_id` | 默认当前活跃租户；`system-admin` 可选任意租户 |
...
2. 分配 `id`、生成 `mlservice_name = "ws-" + crockford32(rand40bit)` 作为 desired CR 名、派生 `pvc_name = "axisml-ws-" + id 前 8 字符 + "-data"`；
3. Platform 直连 K8s 创建 PVC（[§8.5](#85-pvc-管理)）：`name=pvc_name`, `namespace=compute_namespace`, `accessModes=[ReadWriteOnce]`, `resources.requests.storage=<pvcSize>`, 默认 StorageClass；
4. 调 `compute.CreateService(compute_namespace, body)`，body 形如：
   ```jsonc
   {
     "name": "<mlservice_name>",
   ...
   ```
5. Compute 同步返回 service 对象（其中 `id` 是 Compute 生成的 uuid）；写本地 `workspaces` 行 `(id=Platform 自分配, tenant_id, service_id=service.id)`；
6. 失败补偿：见 [§8.2](#82-一致性策略pvc--mlservice--本地-pg)。

响应：`Workspace` DTO（结构与 [§7.1.GET](#get-apiv1workspacesid已登录按角色裁剪) 一致）。

#### `GET /api/v1/workspaces`（已登录，按角色裁剪）

按 [§3.3](#33-列表查询路径) 走 N+1 优化路径。支持 query：`status` / `tenantId` / `q`（关键字）/ `ownerUser`（admin only）/ `limit` / `continue`。
...
合并器：把 Compute `GetServiceByID(service_id)` 返回 + Platform `workspaces` 行 + 可选的实时 PVC GET 三者合成。
...
1. 软删 Platform `workspaces` 行（`deleted_at = now()`）；
2. 调 `compute.DeleteService(compute_namespace, mlservice_name)`（先用 service_id GET 拿 `(ns, name)` 再调 namespace-scoped DELETE，与 [§3.4](#34-对-core-层的硬依赖必须同-pr-推进) B 一致）；
3. 视 `deletePvc`：
   - `true` → 直接 `kubectl delete pvc <pvc_name>`；
   - `false` → 保留 PVC，记录到「孤儿 PVC」管理界面（[§11](#11-后续迭代)）。

DELETE 是幂等的：再次 DELETE 已删除的 workspace 直接返 200。
...
- 先 `GetServiceByID(service_id)` 拿 `(ns, name)` + 当前 `replicas`；
...
跳进去后镜像内的 web server 接管路由；base-url 通过 `AXISML_WORKSPACE_BASE_URL` env 已注入容器，符合 [§3.4 C](#34-对-core-层的硬依赖必须同-pr-推进) 的镜像约定即可正常工作。
...
| `service.go` | 业务编排：创建（PVC + MLService + 本地 PG 三元组）、启停（service_id GET → /scale）、删除（service_id GET → DELETE → PVC GC）、access JWT 颁发；状态读取走「实时 join」非缓存 |
| `repository.go` | GORM 操作 `workspaces` 表（3 业务字段，操作很薄） |
| `dto.go` | 请求 / 响应类型；与 Compute API DTO 之间的显式映射 |
| `view.go` | `Workspace` DTO 合并器：`workspaces` 行 + `compute.GetServiceByID` 结果 + 可选的实时 PVC 数据 → 单一 DTO；含派生 `status` / `desired_state` / `access_url` 的纯函数 |
...
风格沿用 [overview.md §7.1](overview.md#71-仓库与目录布局) 与 [components/compute/](../../../components/compute/) 的 handler/service/repository 三层。
...
### 8.2 一致性策略（PVC + MLService + 本地 PG）

Compute / K8s 是权威；本地 `workspaces` 仅承载视图映射。一致性按「权威先行」组织，跨三个对象的写顺序与失败补偿如下。

**创建**（按 [§7.1 POST](#post-apiv1workspaces已登录--userself-及以上) 顺序）：

1. PVC 创建失败 → 直接返 5xx + problem，无 MLService、无 PG 行；用户重试时由于 `pvc_name = axisml-ws-<id8>-data` 的派生关系，下次的 id 会派生出新的 pvc_name，不会撞名。
2. MLService 创建失败 → 立即 delete PVC 回滚，返 5xx；如果 delete PVC 也失败，PVC 进「孤儿」状态（[§11](#11-后续迭代) 清理）。
3. 本地 PG 写入失败 → 不回滚 PVC / MLService（与 [tenant.md §8.2](tenant.md#82-一致性策略双写-cluster-manager--本地-pg) 同策略：上游权威已生效，PG 行作为视图映射可由后台补偿写入）；返回 5xx + problem `type=https://axisml.io/errors/workspace-record-failed`，提示「MLService 已创建但视图映射写入失败」；inline retry 一次后由 [§11](#11-后续迭代) 持久化补偿队列接管。

**删除**（按 [§7.1 DELETE](#delete-apiv1workspacesidowner--tenant-adminself--system-admin) 顺序）：

1. 软删 PG 行；
2. 调 `compute.DeleteService` 失败 → 4xx 透传 / 5xx 标 problem 但不回滚 PG（PG 已是软删，下次列表视为已删）；
3. PVC delete 失败 → 留作孤儿，登记到 [§11](#11-后续迭代)。

**启停**：单点写 Compute `/scale`，无跨对象副作用；失败直接透传 4xx / 5xx，无补偿。

**孤儿处理**：详情 GET 时若 `compute.GetServiceByID` 返回 404、本地 PG 行存在 → Platform 把该行也软删并返 410，UI 列表自然不再展示。

### 8.3 状态读取与派生

- 任何展示运行态字段的请求都调 `compute.GetServiceByID(service_id)`，不本地缓存；不引入 K8s informer。
- 列表走 `compute.ListServicesByIDs(ids)` 批量端点（[§3.4 B](#34-对-core-层的硬依赖必须同-pr-推进)），单租户百级 workspace 一次 RPC 拉完。
- 派生 `status` 规则（输入：`(workspaces.deleted_at, services.status, services.replicas, services.ready_replicas)`）：

| 条件 | Workspace `status` |
| --- | --- |
| `workspaces.deleted_at IS NOT NULL` 且 `services.status != Deleted` | `Deleting` |
| `workspaces.deleted_at IS NOT NULL` 且 `services.status == Deleted` | `Deleted` |
| `services.status == Creating` | `Creating` |
| `services.replicas == 0` | `Stopped` |
...
- `platform_workspace_state{tenant_id, state}`：gauge，按租户聚合各派生 `status` 的 workspace 数；定期采样写入。
- `platform_workspace_pvc_orphan_total`：counter，记录 [§8.2](#82-一致性策略pvc--mlservice--本地-pg) 中失败补偿留下的孤儿 PVC 数。
- `platform_workspace_access_jwt_issued_total{result}`：counter，access JWT 颁发量 + 失败原因。

zap 字段约定：每条 workspace 操作日志必带 `workspace_id` / `tenant_id` / `service_id` / `actor_user` / `action` / `status`；启停 / 删除额外带 `target_replicas` / `delete_pvc`（如适用）。
...
| Compute id-based 端点 | [§3.4 B](#34-对-core-层的硬依赖必须同-pr-推进)：Compute 实现 `GET /api/v1/services/{id}` + `GET /api/v1/services?ids=` | Compute integration 覆盖 |
| MLService spec 扩展 | [§3.4 A](#34-对-core-层的硬依赖必须同-pr-推进)：MLService spec 加 `volumes` / `volumeMounts`；`(native, deployment)` handler 透传 | compute-operator integration 覆盖 |
| Integration | testcontainers PG + httptest 驱动 in-process gin + envtest 创建 PVC + httptest fake Compute；happy path：创建（jupyter 镜像）→ 启动 → MLService Ready → 停止 → 重新启动数据保留 → 删除（含 PVC） | `make platform-integration` 通过 |
...
4. 持久化补偿 outbox：把 [§8.2](#82-一致性策略pvc--mlservice--本地-pg) 的 inline retry 替换为 PG outbox 表 + 后台 worker。
5. 列表分页 / 排序优化（按租户大小自适应）。
...
| 底层 join key | 用 Compute `services.id`（uuid）而非 `(namespace, name)` | 与 [tenant.md](tenant.md) 用 `tenants.id` 当外键的范式一致；rename 抗性强；列表 N+1 优化天然存在 |
| Platform PG 字段集 | 仅 `(id, tenant_id, service_id)` 3 业务字段 | 与 [overview.md §9](overview.md#9-关键设计决策) 「Platform PG 仅存身份与视图映射」对齐，避免双写漂移 |
| 不缓存 namespace / name | 不存 `compute_namespace` / `mlservice_name` | 与 [tenant.md §3.1](tenant.md#31-tenants-表) 一致；id-based GET 实时返回 |
...
| 持久化 | MVP 内置 PVC；Platform 直管而非塞进 MLService handler | 隔离 workspace 存储语义；MLService 仅负责 `volumes/volumeMounts` 透传，不引入 workspace 专用字段 |
...
| 状态派生 | 基于 `(workspaces.deleted_at, services.status, services.replicas)` 三元组实时计算 | 不缓存避免漂移；与 Compute 的非终态语义一致 |
...
- **审计日志 Tab**：复用 [overview.md §7.4](overview.md#74-pg-schema) 的 `audit_logs` 表，按 `workspace_id` 索引展示。
- **持久化补偿 outbox**：把 [§8.2](#82-一致性策略pvc--mlservice--本地-pg) 中的 inline retry 替换为 PG outbox + 后台 worker。
...
- **Compute service list 按 name 前缀过滤**：当某租户 workspace 数过大时（千级），用 `name=ws-*` 过滤参数比按 id 集合批量 GET 更高效。
...
- **integration**（`components/platform/backend/test/integration/`）：
  - testcontainers PostgreSQL + envtest（用于创建 PVC）+ in-process gin engine（`httptest`）+ httptest fake Compute（按 [§3.4 B](#34-对-core-层的硬依赖必须同-pr-推进) 模拟 id-based 端点）；
  - happy path：创建（jupyter 镜像）→ MLService 出现并 Ready → 停止 → MLService `replicas=0` → 重新启动 → PVC 数据保留校验 → 删除（含 / 不含 PVC）；
  - 故障注入：PVC 创建失败 → 不留 MLService；MLService 创建失败 → PVC 被回滚；本地 PG 写入失败 → MLService 仍存在并由后台补偿；
  - RBAC：`system-admin` / `tenant-admin@self` / `user@self & @owner` / `user@self & 非 owner` 在每个 endpoint 上的允许 / 拒绝矩阵；
  - 列表 N+1 优化：构造 30 个 workspace，断言只调一次 `compute.ListServicesByIDs`。
