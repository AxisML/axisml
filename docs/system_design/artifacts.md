...
| **mlservice-operator** | 通过 Artifacts client SDK 解析 `spec.modelRef` → KServe `storageUri` / `AXISML_MODEL_URI` env | 见 [operator.md §5 MLService Controller](operator.md#6-mlservice-controller) `spec.modelRef` 字段语义 |
