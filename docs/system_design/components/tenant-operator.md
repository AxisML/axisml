...
| 类别 | 内容 |
| --- | --- |
| 输入 CR | `Tenant`（`axisml.io/v1alpha1`, cluster-scoped, `shortName=tnt`）；schema 见 [tenant-crd.yaml](../../../deploy/helm/axisml-system/crds/tenant-crd.yaml) |
...
**展示性元数据归属**：`display_name` / `description` 的权威在上游 `tenants` 表（[database.md §4](../database.md#4-platform)），**不**下发到 Tenant CR；tenant-operator 与 cluster-manager 都不感知。租户的 `labels` / `annotations` 经 cluster-manager 透传到 CR `metadata`，但 tenant-operator 不据此 reconcile、不影响 phase。
...
| Helm / 镜像 | `tenantOperator.*`，见 [deployment.md](../deployment.md) |
