...
- **bootstrap**（`axisml-platform bootstrap`，首次安装）：创建内置角色；创建初始 `system-admin` 账号 `admin`/`admin`（**首登强制改密**，可由 `AXISML_BOOTSTRAP_PASSWORD` 覆盖）；经 cluster-manager 导入 System 层已定义的租户（内置 `default` 的 K8s Namespace 为 `axisml-tenant`，承载 `visibility=public` 制品）。
