...
// axisml-artifacts needs against zot: HEAD manifest (for digest verification
// at complete time), DELETE manifest (for GC), and credential issuance for
// cli push / pull.
//
// MVP simplification (per docs/system_design/artifacts.md §8.3 #4): credential
// issuance is admin-htpasswd passthrough. The artifacts pod holds zot admin
// credentials and returns them verbatim on initiate / resolve?usage=download.
