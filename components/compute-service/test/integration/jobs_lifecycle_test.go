...
// seed ResourcePool CR → POST /jobs → DB row → reconciler tick →
// MLJob CR in envtest. Then GET, List, conflict, Delete.
func TestJobCreateRoundTrip(t *testing.T) {
	...
	body := buildJobCreateBody("my-job", "jobs-e2e-pool", "small")
