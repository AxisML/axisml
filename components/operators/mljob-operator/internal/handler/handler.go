...
// Compute writes into MLJob.spec.backend exactly.
type Key struct {
	Backend string
	Engine  string
}
...
//     watch events to compute the next MLJob.status without round-trips.
type Handler interface {
	Key() Key

	Validate(*axisv1alpha1.MLJob) field.ErrorList
	...
	Reconcile(ctx context.Context, c client.Client, mlJob *axisv1alpha1.MLJob) (underlying any, result ReconcileResult, err error)
	...
	// MLJob reconciles via ownerReference reverse-lookup. Returned
