...
// MLJobReconciler is the dispatcher Reconciler. It owns the only write
// path to MLJob.status: handlers return MapStatusResult/ReconcileResult
...
func (r *MLJobReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	logger := log.FromContext(ctx).WithValues("mljob", req.NamespacedName)

	var mlJob axisv1alpha1.MLJob
...
func (r *MLJobReconciler) writeStatus(
	ctx context.Context,
	...
	if err := r.Status().Patch(ctx, mlJob, patch); err != nil {
		return fmt.Errorf("patch MLJob status: %w", err)
	}
	return nil
}
...
func (r *MLJobReconciler) assertSpecImmutable(ctx context.Context, mlJob *axisv1alpha1.MLJob) error {
...
// specFingerprint canonicalises the immutable portion of MLJob.spec
...
func specFingerprint(mlJob *axisv1alpha1.MLJob) string {
	snapshot := struct {
...
func statusEqual(a, b axisv1alpha1.MLJobStatus) bool {
	return equality.Semantic.DeepEqual(a, b)
}

// SetupWithManager wires the dispatcher to MLJob events and to every
// underlying GVK any registered handler watches. ownerReference reverse
// lookup (Owns) requeues MLJob keys when a derived resource changes.
...
func (r *MLJobReconciler) SetupWithManager(mgr ctrl.Manager) error {
	b := ctrl.NewControllerManagedBy(mgr).
		For(&axisv1alpha1.MLJob{}, builder.WithPredicates(predicate.GenerationChangedPredicate{}))
	...
	for _, h := range r.Registry.All() {
		for _, target := range h.WatchTargets() {
			...
			seen[gvk] = struct{}{}
			b = b.Watches(target,
				handler.EnqueueRequestForOwner(mgr.GetScheme(), mgr.GetRESTMapper(), &axisv1alpha1.MLJob{}, handler.OnlyControllerOwner()))
		}
	}
	return b.Complete(r)
}

var _ reconcile.Reconciler = (*MLJobReconciler)(nil)
