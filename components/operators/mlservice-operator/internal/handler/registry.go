...
// Register adds a factory. Panics on duplicate keys — duplicate registrations
// indicate a programming error at process startup, not a runtime condition.
func Register(key Key, f Factory) {
	registryMu.Lock()
	defer registryMu.Unlock()
	if _, exists := factories[key]; exists {
		panic(fmt.Sprintf("handler: duplicate registration for %s", key))
	}
	factories[key] = f
}

// Keys returns every registered (backend, engine) tuple. Order is not
...
// Build instantiates every registered factory against the manager. Returns a
// map keyed by (backend, engine) plus the flat slice for iteration.
