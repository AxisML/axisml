...
import (
	...
	"github.com/axisml/axisml/components/compute-operator/internal/mljob/handler"
)
...
func TestMergeStatus_TerminalPhasePrecedence(t *testing.T) {
	...
	existing := axisv1alpha1.MLJobStatus{Phase: axisv1alpha1.PhaseSucceeded}
...
func TestMergeStatus_FreshTerminalAccepted(t *testing.T) {
	...
	existing := axisv1alpha1.MLJobStatus{Phase: axisv1alpha1.PhaseRunning}
	now := metav1.Now()
	got := mergeStatus(existing,
...
func TestMergeStatus_SuspendCondition(t *testing.T) {
	...
	existing := axisv1alpha1.MLJobStatus{Phase: axisv1alpha1.PhasePending}
	got := mergeStatus(existing,
